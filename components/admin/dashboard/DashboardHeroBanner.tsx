...
export default function DashboardHeroBanner({
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden shadow-lg border border-slate-800/40 bg-slate-950 text-white min-h-[170px] sm:min-h-[190px] flex flex-col justify-between p-6 sm:p-8 transition-all">
      ...
      <div
        className="absolute inset-0 z-0 bg-cover bg-center transition-transform duration-700 ease-out hover:scale-105"
        style={{
          backgroundImage: `url(${imageUrl})`,
        }}
      />
      <div className="absolute inset-0 z-0 bg-gradient-to-r from-slate-950/95 via-slate-950/80 to-slate-950/50 backdrop-blur-[0.5px]" />
      <div className="absolute inset-0 z-0 bg-radial-at-tl from-amber-500/10 via-transparent to-transparent" />

      {/* Top Row: Tagline + Dynamic Customizer Trigger */}
      <div className="relative z-10 flex items-center justify-between gap-4">
        ...
        {canCustomize && onOpenCustomizer && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold text-slate-300 hover:text-white bg-slate-900/60 hover:bg-slate-800/90 border border-white/10 backdrop-blur-md transition-all cursor-pointer group shadow-xs"
            title="Edit hero quotes, images and pillars"
          >
            <Edit3 size={13} className="text-slate-400 group-hover:text-amber-400 transition-colors" />
            <span>Customize</span>
          </button>
        )}
      </div>
