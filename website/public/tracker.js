(function () {
  try {
    // ── Exclude localhost, loopback, and internal admin pages ───────────────
    ...
    document.addEventListener(
      ...
      function (e) {
        ...
        if (!anchor && el && el.tagName === 'A' && el.href && el.href.startsWith('tel:'))
          anchor = el;
        if (anchor) {
          var callText = (anchor.innerText || anchor.getAttribute('aria-label') || anchor.getAttribute('title') || 'Phone Call').trim().replace(/\s+/g, ' ').slice(0, 80);
          send({ eventType: 'call', label: callText || 'Phone Call', element: 'a[tel]' });
          return;
        }
