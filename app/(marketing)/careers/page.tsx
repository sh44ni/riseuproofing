      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
        {jobs.map((job) => (
          <Link key={job.slug} href={`/careers/${job.slug}`} className="group flex">
            <div className="glass-card-interactive rounded-2xl p-6 sm:p-7 flex flex-col justify-between w-full border border-white/15 hover:border-white/30 transition-all duration-300">
              <div>
                <div className="flex items-start justify-between gap-4 mb-3">
                  <h3 className="text-lg font-bold text-white group-hover:text-brand-blue transition-colors">
                    {job.title}
                  </h3>
                  <span className="glass-chip px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider text-emerald-300 bg-emerald-500/10 border-emerald-400/30">
                ...
                <p className="text-xs text-white/75 line-clamp-2 mb-6 leading-relaxed">
                  {job.description}
                </p>
              </div>
