...
export const STAGES = [
  ...
  { id: 'in_progress', label: 'In Progress', color: 'border-amber-500/40 text-amber-400 bg-amber-500/10' },
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-900/60 border border-white/10 rounded-2xl p-4">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Active Work Value</p>
        ...
        <div className="bg-slate-900/60 border border-white/10 rounded-2xl p-4">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Completed Jobs</p>
        ...
        <div className="bg-slate-900/60 border border-white/10 rounded-2xl p-4">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Total Contract Volume</p>
          <p className="text-2xl font-extrabold text-emerald-400 mt-1 tabular-nums">
      ...
      <div className="space-y-3 bg-slate-900/60 p-3.5 sm:p-4 rounded-2xl border border-white/10">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-800 border border-white/10 rounded-xl text-white placeholder-slate-500 text-sm focus:outline-none focus:border-amber-400"
          />
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3">
          ...
          <p className="text-slate-400 text-sm">Loading roofing jobs...</p>
        </div>
      ) : (
        <>
          ...
          <div className="lg:hidden space-y-3">
            ...
            {jobs.filter(j => activeTab === 'all' || j.status === activeTab).length === 0 && (
              <div className="py-12 text-center text-slate-500 text-sm bg-slate-900/40 rounded-2xl border border-white/5">
          ...
          <div className="hidden lg:grid grid-cols-7 gap-3 overflow-x-auto min-w-[1300px] pb-4">
            {STAGES.map(stage => {
              ...
              return (
                <div
                  key={stage.id}
                  className="bg-slate-900/50 border border-white/10 rounded-2xl p-3 flex flex-col min-h-[500px]"
                >
                  {/* Column Header */}
                  <div className="flex items-center justify-between pb-3 border-b border-white/5 mb-3">
                    <span className="text-xs font-bold text-slate-300 truncate">{stage.label}</span>
                    <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-amber-400">
                      {stageJobs.length}
                    </span>
                  </div>
      ...
      >
        <form onSubmit={handleCreateManualJob} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Crew Lead</label>
              <input
                type="text"
                value={manualLead}
...
function JobCard({
  ...
  return (
    <div className="bg-slate-800/90 hover:bg-slate-800 border border-white/10 hover:border-white/20 rounded-2xl p-3.5 space-y-2.5 shadow-sm transition-all group">
      <div className="flex items-start justify-between gap-1">
        <span className="font-mono text-[10px] font-bold text-amber-400/90 tracking-wide">
          {job.job_number}
        </span>
        <select
          value={job.status}
          onChange={e => onStageChange(job.id, e.target.value)}
          className="text-[10px] font-bold px-2 py-0.5 rounded-full border border-white/10 bg-slate-900 text-slate-300 cursor-pointer outline-none"
        >
          {STAGES.map(s => (
