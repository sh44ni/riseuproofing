...
(function(){
  try {
    ...
    utmKeys.forEach(function(k){ var v = sp.get(k); if(v){ utms[k.replace('utm_','')] = v; sessionStorage.setItem('_'+k, v); } });
    // Restore UTMs from session if not in current URL
    if(!utms.source){ utmKeys.forEach(function(k){ var v=sessionStorage.getItem('_'+k); if(v) utms[k.replace('utm_','')] = v; }); }
    ...
    document.addEventListener('click', function(e) {
      ...
      for(var i=0;i<3;i++){ if(!el||el===document.body) break; if(el.tagName==='A'||el.tagName==='BUTTON') break; el=el.parentElement; }
      var tag = el ? el.tagName : '';
      var label = (el && (el.getAttribute('aria-label') || el.innerText || el.getAttribute('data-label') || el.className || tag) || '').trim().slice(0,80);
      var xPct = (e.clientX / window.innerWidth * 100).toFixed(1);
      var yPct = ((e.clientY + window.scrollY) / Math.max(document.body.scrollHeight,1) * 100).toFixed(1);

      // Call click
      var anchor = el && el.closest ? el.closest('a[href^="tel:"]') : null;
      if (!anchor && el && el.tagName === 'A' && el.href && el.href.startsWith('tel:')) anchor = el;
      if (anchor) { send({ eventType: 'call', label: 'Phone Call' }); return; }

      // Outbound link
      if (tag === 'A' && el.href && el.hostname && el.hostname !== location.hostname) {
        send({ eventType: 'outbound_link', label: el.href.slice(0,120), element: label });
        return;
      }

      // Nav click
      var isNav = el && (el.closest('nav') || el.closest('header'));
      if (isNav) { send({ eventType: 'nav_click', label: label, xPct: parseFloat(xPct), yPct: parseFloat(yPct) }); return; }

      // Button / CTA click
      if (tag === 'BUTTON' || tag === 'A') {
        send({ eventType: 'button_click', label: label, element: tag, xPct: parseFloat(xPct), yPct: parseFloat(yPct) });
        return;
      }

      // Generic click (heatmap)
      send({ eventType: 'click', element: label, xPct: parseFloat(xPct), yPct: parseFloat(yPct) });
    }, { passive: true });
    ...
    window.addEventListener('scroll', function() {
      ...
      [25,50,75,90,100].forEach(function(t){
        if (pct >= t && !reported[t]) { reported[t]=1; send({ eventType: 'scroll', scrollPct: t }); }
      });
    }, { passive: true });
    ...
    document.addEventListener('focusin', function(e) {
      ...
      if (!form) return;
      var fid = form.id || form.getAttribute('name') || form.className.split(' ')[0] || 'form';
      if (!formStarted[fid]) {
        formStarted[fid] = 1;
    ...
    function sendSessionEnd() {
      ...
      send({ eventType: 'session_end', durationMs: dur, label: Math.round(dur/1000)+'s' });
    }
